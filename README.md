...
На эти дорожки **копируется** звук, приходящий в микшер, этот звук там тоже как-то обрабатывается и то, что получилось отправляется **Master** канал, и там это всё сливается в один звук. 
...
- **Ext. In** это обычно внешняя звуковая карта
- **Configure** позволяет быстро открыть меню выбора входного звукового устройства
- **Resampling** позволяет записать звук со всех дорожек в одну. Для этого нужно:
  1. Создать пустой **Audio Track**
  2. Выбрать на нём **Resampling** в качестве входного сигнала
  3. Включить запись на дорожке
  4. Записать звук
  5. Теперь у нас есть дородка, на которой записана сумма всех других дорожек. Наверное это зачем-то нужно :) ![image](./images/resampling.png)
